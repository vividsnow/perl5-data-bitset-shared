...
#define BS_VERSION     1
#define BS_ERR_BUFLEN  (PATH_MAX + 256)
...
static int bs_file_is_zero(int fd, uint64_t size) {
    char buf[16384];
...
static int bs_reserve(int fd, uint64_t size) {
    const char *sp = getenv("DATA_BITSET_SHARED_SPARSE");
...
static BsHandle *bs_create(const char *path, uint64_t capacity, mode_t mode, char *errbuf) {
    ...
    if (anonymous) {
        ...
        if (is_new && bs_reserve(fd, total) < 0) {
            BS_ERR("%s: cannot reserve %llu bytes: %s", path, (unsigned long long)total, strerror(errno));
        ...
        if (!is_new) {
            if (!bs_validate_header((BsHeader *)base, (uint64_t)st.st_size)) {
                ...
                if (((BsHeader *)base)->magic == 0 && (uint64_t)st.st_size == total
                    && st.st_uid == geteuid() && bs_file_is_zero(fd, map_size)) {
                    if (fchmod(fd, mode) < 0) {
                        BS_ERR("%s: fchmod: %s", path, strerror(errno));
                        munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                    }
                    if (bs_reserve(fd, total) < 0) {
                        BS_ERR("%s: cannot reserve %llu bytes: %s", path, (unsigned long long)total, strerror(errno));
                        munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                    }
...
static BsHandle *bs_create_memfd(const char *name, uint64_t capacity, char *errbuf) {
    ...
    if (bs_reserve(fd, total) < 0) {
        BS_ERR("memfd: cannot reserve %llu bytes: %s", (unsigned long long)total, strerror(errno));
...
static BsHandle *bs_open_fd(int fd, char *errbuf) {
    ...
    uint32_t magic;
    if (pread(fd, &magic, sizeof magic, offsetof(BsHeader, magic)) != (ssize_t)sizeof magic || magic != BS_MAGIC) {
        BS_ERR("invalid bitset"); return NULL;
    }
...
static BsHandle *bs_open_readonly(const char *path, char *errbuf) {
    ...
    uint32_t magic;
    if (pread(fd, &magic, sizeof magic, offsetof(BsHeader, magic)) != (ssize_t)sizeof magic || magic != BS_MAGIC) {
        BS_ERR("%s: invalid bitset file", path); close(fd); return NULL;
    }
    size_t ms = (size_t)st.st_size;
